...
Instead of using the [thumbnail](https://github.com/eBollow05/vcard-downloads/blob/main/functions.php#L18) for the profile picture, you also could use a meta field for that. But don't forget to wrap it's value with `wp_get_attachment_image_url()` if the field saves the values as image ID instead of URL.
